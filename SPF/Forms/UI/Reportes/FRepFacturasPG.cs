...
{
    public partial class FRepFacturasPG : Form
    {
        #region Variables
        ...
        public FRepFacturasPG()
        {
            InitializeComponent();
        }

        public FRepFacturasPG(string titulo, BerkeDBEntities context)
        {
            InitializeComponent();
            this.Text = titulo;
        ...
        {
            List<GetListaFacturasPG_Result> reportDS = new List<GetListaFacturasPG_Result>();

            try
            {
                reportDS = this.DBContext.GetListaFacturasPG(ProveedorID,
                                                                  FechaDesde,
                                                                  FechaHasta,
                                                                  MonedaID).ToList();
            }
            ...
            string path = @"Reportes\RepFacturasPG.rdlc";
            string asuntoMail = "Reporte de Facturas de Proveedores - Práctica General - Detallado";
            string cuerpoMail = "Por favor revise el documento adjunto.";
            string nombreArchivo = "RepFacturasProvPGDetallado";

            FReportViewerBase f = new FReportViewerBase(path, datasource, this.DBContext);
            f.FormClosed += delegate { f = null; };
            
            f.Titulo = String.Format("Reporte de Facturas de Proveedores - Práctica General  - Detallado - {0}", reportDS.First().Rango);
            f.NombreArchivoAdjunto = nombreArchivo;
            f.AsuntoMail = asuntoMail + " - " + String.Format("Reporte de Facturas de Proveedores - Práctica General  - Detallado - {0}", reportDS.First().Rango);
        ...
        {
            List<GetListaFacturasConsoPG_Result> reportDS = new List<GetListaFacturasConsoPG_Result>();

            try
            {
                reportDS = this.DBContext.GetListaFacturasConsoPG(ProveedorID,
                                                                  FechaDesde,
                                                                  FechaHasta,
                                                                  MonedaID).ToList();
            }
            ...
            string path = @"Reportes\RepFacturasConsoPG.rdlc";
            string asuntoMail = "Reporte de Facturas de Proveedores - Práctica General - Consolidado";
            string cuerpoMail = "Por favor revise el documento adjunto.";
            string nombreArchivo = "RepFacturasProvConsoPG";

            FReportViewerBase f = new FReportViewerBase(path, datasource, this.DBContext);
            f.FormClosed += delegate { f = null; };
            f.Titulo = String.Format("Reporte de Facturas de Proveedores - Práctica General - Consolidado - {0}", reportDS.First().Rango);
            f.NombreArchivoAdjunto = nombreArchivo;
            f.AsuntoMail = asuntoMail + " - " + String.Format("Reporte de Facturas de Proveedores - Práctica General - Consolidado - {0}", reportDS.First().Rango);
            f.CuerpoMail = cuerpoMail;
            f.ShowDialog();
        }
