...
{
    public partial class FRepEstadoCtaGcial : Form
    {
        #region Variables
        ...
        public FRepEstadoCtaGcial()
        {
            InitializeComponent();
        }

        public FRepEstadoCtaGcial(string titulo, BerkeDBEntities context)
        {
            InitializeComponent();
            this.Text = titulo;
        ...
        private void btnAceptar_Click(object sender, EventArgs e)
        {
            ...
            List<GetEstadoCuentaGerencial_Result> reportDS = new List<GetEstadoCuentaGerencial_Result>();

            try
            {
                string MonedaID = null;
                decimal tCambio = Convert.ToDecimal(this.txtTipoCambio.Text);
                reportDS = this.DBContext.GetEstadoCuentaGerencial(this.txtFechaDesde.Text, 
                ...
                string path = @"Reportes\RepEstadoCtaGerencial.rdlc";
                string asuntoMail = "Reporte de Cuentas a Pagar - Gerencial";
                string cuerpoMail = "Por favor revise el documento adjunto.";
                string nombreArchivo = "RepEstadoCtaGerencial";

                FReportViewerBase f = new FReportViewerBase(path, datasource, this.DBContext);
                f.FormClosed += delegate { f = null; };
                f.Titulo = String.Format("Reporte de Cuentas a Pagar - Gerencial - Al {0}", this.txtFechaDesde.Text);
                f.NombreArchivoAdjunto = nombreArchivo;
                f.AsuntoMail = asuntoMail + " - " + String.Format("Reporte de Cuentas a Pagar - Gerencial - Al {0}", this.txtFechaDesde.Text);
