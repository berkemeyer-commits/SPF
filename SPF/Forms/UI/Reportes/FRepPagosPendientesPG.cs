...
{
    public partial class FRepPagosPendientesPG : Form
    {
        #region Variables
        ...
        public FRepPagosPendientesPG()
        {
            InitializeComponent();
        }

        public FRepPagosPendientesPG(string titulo, BerkeDBEntities context)
        {
            InitializeComponent();
            this.Text = titulo;
        ...
        {
            List<GetListaPagosPendientesPG_Result> reportDS = new List<GetListaPagosPendientesPG_Result>();

            try
            {
                reportDS = this.DBContext.GetListaPagosPendientesPG(ProveedorID,
                                                                  FechaDesde,
                                                                  FechaHasta,
                                                                  MonedaID).ToList();
            }
            ...
            string path = @"Reportes\RepPagosPendientesPG.rdlc";
            string asuntoMail = "Reporte de Pagos Pendientes - Práctica General - Detallado";
            string cuerpoMail = "Por favor revise el documento adjunto.";
            string nombreArchivo = "RepPagosPendientesPGDetallado";

            FReportViewerBase f = new FReportViewerBase(path, datasource, this.DBContext);
            f.FormClosed += delegate { f = null; };
            f.Titulo = String.Format("Reporte de Pagos Pendientes - Práctica General - Detallado - {0}", reportDS.First().Rango);
            f.NombreArchivoAdjunto = nombreArchivo;
            f.AsuntoMail = asuntoMail + " - " + String.Format("Reporte de Pagos Pendientes - Práctica General - Detallado - {0}", reportDS.First().Rango);
        ...
        {
            List<GetListaPagosPendientesConsoPG_Result> reportDS = new List<GetListaPagosPendientesConsoPG_Result>();

            try
            {
                reportDS = this.DBContext.GetListaPagosPendientesConsoPG(ProveedorID,
                                                                  FechaDesde,
                                                                  FechaHasta,
                                                                  MonedaID).ToList();
            }
            ...
            string path = @"Reportes\RepPagosPendientesConsoPG.rdlc";
            string asuntoMail = "Reporte de Pagos Pendientes - Práctica General - Consolidado";
            string cuerpoMail = "Por favor revise el documento adjunto.";
            string nombreArchivo = "RepPagosPendientesPGConso";

            FReportViewerBase f = new FReportViewerBase(path, datasource, this.DBContext);
            f.FormClosed += delegate { f = null; };
            f.Titulo = String.Format("Reporte de Pagos Pendientes - Práctica General - Consolidado - {0}", reportDS.First().Rango);
            f.NombreArchivoAdjunto = nombreArchivo;
            f.AsuntoMail = asuntoMail + " - " + String.Format("Reporte de Pagos Pendientes- Práctica General -  Consolidado - {0}", reportDS.First().Rango);
            f.CuerpoMail = cuerpoMail;
            f.ShowDialog();
        }
