...
{
    public partial class FRepPagosPendientes : Form
    {
        #region Variables
        ...
        public FRepPagosPendientes()
        {
            InitializeComponent();
        }

        public FRepPagosPendientes(string titulo, BerkeDBEntities context)
        {
            InitializeComponent();
            this.Text = titulo;
        ...
        {
            List<GetListaPagosPendientes_Result> reportDS = new List<GetListaPagosPendientes_Result>();

            try
            {
                reportDS = this.DBContext.GetListaPagosPendientes(ProveedorID,
                                                                  FechaDesde,
                                                                  FechaHasta,
                                                                  MonedaID).ToList();
            }
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Ocurrió un error al generar el reporte: " + Environment.NewLine + ex.Message + Environment.NewLine + innerException,
            ...
            string path = @"Reportes\RepPagosPendientes.rdlc";
            string asuntoMail = "Reporte de Pagos Pendientes Detallado";
            string cuerpoMail = "Por favor revise el documento adjunto.";
            string nombreArchivo = "RepPagosPendientesDetallado";

            FReportViewerBase f = new FReportViewerBase(path, datasource, this.DBContext);
            f.FormClosed += delegate { f = null; };
            f.Titulo = String.Format("Reporte de Pagos Pendientes Detallado - {0}", reportDS.First().Rango);
            f.NombreArchivoAdjunto = nombreArchivo;
            f.AsuntoMail = asuntoMail + " - " + String.Format("Reporte de Pagos Pendientes Detallado - {0}", reportDS.First().Rango);
        ...
        {
            List<GetListaPagosPendientesConso_Result> reportDS = new List<GetListaPagosPendientesConso_Result>();

            try
            {
                reportDS = this.DBContext.GetListaPagosPendientesConso(ProveedorID,
                                                                  FechaDesde,
                                                                  FechaHasta,
                                                                  MonedaID).ToList();
            }
            ...
            string path = @"Reportes\RepPagosPendientesConso.rdlc";
            string asuntoMail = "Reporte de Pagos Pendientes Consolidado";
            string cuerpoMail = "Por favor revise el documento adjunto.";
            string nombreArchivo = "RepPagosPendientesConso";

            FReportViewerBase f = new FReportViewerBase(path, datasource, this.DBContext);
            f.FormClosed += delegate { f = null; };
            f.Titulo = String.Format("Reporte de Pagos Pendientes Consolidado - {0}", reportDS.First().Rango);
            f.NombreArchivoAdjunto = nombreArchivo;
            f.AsuntoMail = asuntoMail + " - " + String.Format("Reporte de Pagos Pendientes Consolidado - {0}", reportDS.First().Rango);
            f.CuerpoMail = cuerpoMail;
            f.ShowDialog();
        }

        private void GenConsolidado2(string ProveedorID, string FechaDesde, string FechaHasta, string MonedaID)
        {
            List<GetListaPagosPendientesConso2_Result> reportDS = new List<GetListaPagosPendientesConso2_Result>();

            try
            {
                reportDS = this.DBContext.GetListaPagosPendientesConso2(ProveedorID,
                                                                  FechaDesde,
                                                                  FechaHasta,
                                                                  MonedaID).ToList();
            }
            ...
            string path = @"Reportes\RepPagosPendientesConso2.rdlc";
            string asuntoMail = "Reporte de Pagos Pendientes - Listado Completo";
            string cuerpoMail = "Por favor revise el documento adjunto.";
            string nombreArchivo = "RepPagosPendientesConso";

            FReportViewerBase f = new FReportViewerBase(path, datasource, this.DBContext);
            f.FormClosed += delegate { f = null; };
            f.Titulo = String.Format("Reporte de Pagos Pendientes - Listado Completo - {0}", reportDS.First().Rango);
            f.NombreArchivoAdjunto = nombreArchivo;
            f.AsuntoMail = asuntoMail + " - " + String.Format("Reporte de Pagos Pendientes - Listado Completo - {0}", reportDS.First().Rango);
            f.CuerpoMail = cuerpoMail;
            f.ShowDialog();
        }
