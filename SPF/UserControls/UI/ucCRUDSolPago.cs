...
{
    public partial class ucCRUDSolPago : ucBaseForm
    {
        #region Constantes
        //Cabecera
        ...
        private const string CAMPO_EXCLUIRDELISTADOS = "ExcluirDeListados";
        //Detalle
        ...
        public ucCRUDSolPago()
        {
            InitializeComponent();
        }

        public ucCRUDSolPago(string Titulo = "Sin Título", BerkeDBEntities dbContext = null)
        {
            InitializeComponent();
            ...
            var qryB = (from spc in this.DBContext.spc_solicitudpagocab
                        ...
                        join mon in this.DBContext.Moneda
                          on spc.spc_monedaid equals mon.ID /*into mon_spc
                        from mon in mon_spc.DefaultIfEmpty()*/
                        ...
                        join pr in this.DBContext.pr_proveedor
                          on spd.spd_proveedorid equals pr.pr_proveedorid into pr_spd
                        from pr in pr_spd.DefaultIfEmpty()
                        ...
                        select new SolPagoAll
                             {
                                 ...
                                 ExcluirDeListados = spc.spc_excluirlistados,
                                 //Detalle
            ...
            List<int> listaSolicitudesPagoCabIds = new List<int>();
            ...
            //if (this.PuedeVerTodo)
            {
                ...
                listaSolicitudesPagoCabIds = (from s in this.DBContext.spc_solicitudpagocab
                                              where s.spc_origen != ORIGEN_REGEXT
                                              orderby s.spc_solicitudpagocabid descending
                                              select s.spc_solicitudpagocabid)
                                         .Take(50)
                                         .ToList();

                solpag = qryB
                    .Where(b => listaSolicitudesPagoCabIds.Contains(b.SolPagoCabID))
            ...
            {
                //WHERE_PROVEEDOR_PUBLICO = " ((ProveedorPublico = true) OR (ProveedorID == null)) ";
                //solpag = qryB.Where(WHERE_PROVEEDOR_PUBLICO).OrderByDescending(a => a.SolPagoCabID).Take(50).ToList();

                var lSolCabIds = (from s in this.DBContext.spc_solicitudpagocab
                                 join d in this.DBContext.spd_solicitudpagodet
                                   on s.spc_solicitudpagocabid equals d.spd_solicitudpagocabid into s_d
                                 from d in s_d.DefaultIfEmpty()
                                 join p in this.DBContext.pr_proveedor
                                   on d.spd_proveedorid equals p.pr_proveedorid into d_p
                                 from p in d_p.DefaultIfEmpty()
                                 select new
                                 {
                                     SolicitudPagoCabID = s.spc_solicitudpagocabid,
                                     Origen = s.spc_origen,
                                     ProveedorPublico = p.pr_publico,
                                     SolicitudProveedorID = d.spd_proveedorid
                                 })
                                .Where(a => a.Origen != ORIGEN_REGEXT && (a.ProveedorPublico == true || !a.SolicitudProveedorID.HasValue))
                                .GroupBy(a => new { a.SolicitudPagoCabID }).Select(g => g.FirstOrDefault())
                                .OrderByDescending(a => a.SolicitudPagoCabID)
                                .Take(50)
                                .ToList();

                listaSolicitudesPagoCabIds = (from l in lSolCabIds 
                                              select l.SolicitudPagoCabID)
                                              .ToList();
            ...
            var query = (from item in solpag
                         select new SolPagoCab
                         {
                             ...
                             ExcluirDeListados = item.ExcluirDeListados
                         })
            ...
            this.SetFilter(CAMPO_EXCLUIRDELISTADOS, "Excluir de Listados (S/N)", false);
            ...
            this.SetFilter(CAMPO_NOMBREPROVEEDOR, "Nombre Prov.");
            ...
            this.SetFilter(CAMPO_NROFACTURAPROV, "N° Fact. Prov.");
            this.SetFilter(CAMPO_FECFACTURA, "Fec. Fact. Prov.");
            this.TituloBuscador = "Buscar Solicitudes de Pago a Proveedores";
        ...
        protected override void FormatearGrilla()
        {
            ...
            this.dgvListadoABM.Columns[CAMPO_EXPEDIENTEID].Width = 70;
            ...
            this.dgvListadoABM.Columns[CAMPO_MONEDAABREV].Width = 70;
            ...
            this.dgvListadoABM.Columns[CAMPO_TRAMITEDESCRIP].Width = 220;
        ...
        protected override void tbbNuevo_Click(object sender, EventArgs e)
        {
            ...
            this.chkExcluirDeListados.Checked = false;
        ...
        private void tbbEditar_Click_1(object sender, EventArgs e)
        {
            if (this.ExistenPagosAsociados())
            {
                MessageBox.Show("La solicitud ya cuenta con pagos asociados. Sólo se puede editar el apartado de asociación a Presupuestos," + Environment.NewLine +
                                "la Referencia del Cliente o el detalle de facturas no saldadas aún.",
                                "Información",
        ...
        protected override void Editable(bool editar)
        {
            ...
            this.chkExcluirDeListados.Enabled = !editar;
            ...
            else
            {
                ...
                this.btnEliminarDetalle.Enabled = true;
        ...
        {
            if (this.txtSolPagoCabID.Text.Trim() != string.Empty)
            {
        ...
        private void CargarSolPago(DataGridViewRow row)
        {
            ...
            if (row.Cells[CAMPO_EXCLUIRDELISTADOS].Value != null)
            {
                this.chkExcluirDeListados.Checked = (bool)row.Cells[CAMPO_EXCLUIRDELISTADOS].Value;
            }
            else this.chkExcluirDeListados.Checked = false;
        ...
        private spc_solicitudpagocab guardarSolPagoCab(BerkeDBEntities context = null)
        {
            ...
            if (this.FormEditStatus == EDIT)
            {
                ...
                spc.spc_excluirlistados = this.chkExcluirDeListados.Checked;
            }
            else if (this.FormEditStatus == INSERT)
            {
                spc.spc_fechasol = Convert.ToDateTime(this.txtFechaSolCab.Text);
